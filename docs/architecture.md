...
Enoki の設計・内部仕様をまとめた開発者向けドキュメントです。
はじめて読む場合は [README](../README.md) を、キャラクターの差し替えだけが目的なら
[customization.md](customization.md) を先に読んでください。

macOS のデスクトップに常駐するマスコットです。Codex Pet のアセット形式（8 列 × 9 行 / 192×208px）と
**非公式に互換**で、同じ形式のフォルダをそのまま読み込んで透明ウィンドウの中で動かします
（**Enoki は OpenAI とは無関係で、承認・提携・推奨を受けたものではありません。**
Codex / OpenAI 由来の公式アセットはこのリポジトリに含めていません）。
同梱しているのは**デモ用キャラクター「栞」1 体**（`Resources/DefaultSkin/`）で、
`~/.codex/pets/<名前>/` などに置いたスキンに差し替えられます（§6）。
...
| `EnokiCoreTests` | XCTest + `EnokiCore` | 155 件のユニットテスト。 |
...
Sources/Enoki/
  ...
  Resources/DefaultSkin/         内蔵スキン = デモ用キャラクター「栞」（pet.json + spritesheet.png）
  Resources/Profiles/profiles.json  見た目プロファイルの定義（§12）
  Resources/Schedule/           イベント日程の置き場（README のみ。日程データは同梱しない。§12.9）
...
**Codex Pet のアセット形式との非公式互換**であり、OpenAI とは無関係です（承認・提携・推奨を受けていません）。
...
- **会話の台詞は JSON からの選択だけ**です（生成はしません）。同梱のデモは 30 会話しかないので、
...
- **見た目プロファイル用のスプライトセットは 1 つも同梱していません。**
  `default` / `work` はベーススキン（同梱のデモ）を使い、`casual` / `renofa` は素材を置かなければ
...
- **祝日データは同梱の JSON**、**試合日程はローカルに置く JSON** です（アプリは通信しません）。
  祝日は内閣府が翌年分までしか公開しないので、年に一度 `scripts/update_holidays.py` で更新します。
  データに無い年は現行ルールの計算で補うため、一過性の特例（五輪年の移動など）は合いません（§12.9）。
  **試合日程はリポジトリに含めていません**（個人データ扱い）。置いていなければ曜日と祝日だけで
  判定します。シーズンが変わったら自分で差し替えが必要です（§12.9）。
...
2. アプリ内蔵の `Resources/DialogueText/dialogue.json`（デモ用の 30 会話・1 人用）
...
```json
{
  ...
  "speakers": [
    ...
    {"id": "saku", "displayName": "朔", "anchor": 0.25}
  ],
  "dialogues": [
    {
      "id": "water_001",
...
| `speakers` | [Object]? | — | 話者の宣言。`id`（必須） / `displayName`（既定 = id） / `anchor`（0〜1、既定は人数から自動）。**省略可**。 |
...
| `dialogues[].lines[].speaker` | String | **必須** | 話者 id。任意の文字列。`speakers` を宣言した場合はその id のいずれか（違う id はその会話を捨てる）。 |
...
| `renofa_pre_match` | キックオフ **120 分前**から、キックオフまで |
...
- 同梱ファイル（デモ用・1 人）の内容は `DialogueLoaderTests` が検証しています
  （全 30 会話 = 6 カテゴリ × 5、話者は 1 人、`pair` とプロファイル専用の台詞は無し、
  id の重複なし、1 会話 4 発言まで）。件数を変えたらテストの期待値も直してください。
...
- しっぽは発言者の立ち位置に向きます（`dialogue.json` の `speakers[].anchor`。省略時は
  **1 人なら中央 / 2 人なら左 1/4・右 3/4 / 3 人以上は等間隔**）。
- 文字は system font 13pt、最大幅 220pt。行数は制限せず、長い台詞は吹き出しが縦に伸びます
  ...
  発言者名を小さく上に出します（色は `speakers` の並び順に紺 → 赤茶 → 緑 → 紫）。
...
- `renofa-schedule.json` は**リポジトリに含めていません**（個人データ。§12.9）。
  無ければ `DayProfileResolver` は曜日と祝日だけで判定します。
...
```json
{
  ...
  "profiles": [
    ...
    {"id": "renofa", "displayName": "Renofa", "spriteSet": "saku_shiori_renofa",
     "spriteSetsByPhase": {"matchDay": "saku_shiori_renofa_matchday",
...
| `spriteSetsByPhase` | [String: String]? | `null` | 試合日の**局面ごと**のスプライトセット（§12.3・§12.9）。キーは `MatchPhase` の rawValue = `matchDay` / `preMatch` / `inMatch` / `postMatch`。書いていない局面と、試合日でない日は `spriteSet` を使います。`finished`（試合終了想定時刻以降）は `postMatch` のセットのままです。 |
...
- **同梱しているスプライトセットはありません。** `default` / `work` は `spriteSet: null` で
  ベーススキン（同梱のデモ）をそのまま使い、`casual` は素材を置かなければベーススキンのままです。
  朔と栞のセット（`saku_shiori_casual` など）は開発者個人のアセットなので配布していません
  （`~/Library/Application Support/Enoki/Characters/` に置けば使われます）。
...
試合日は、**キックオフからの経過で `renofa` プロファイルのスプライトセットを差し替えます**
（`profiles.json` の `spriteSetsByPhase`。§12.9）。ポーズだけが違う 4 セットを使います。

| 局面 | いつ | スプライトセット |
|---|---|---|
| `matchDay` | 00:00 〜 キックオフ **2 時間前**（キックオフ不明なら終日） | `saku_shiori_renofa_matchday` |
| `preMatch` | キックオフ **2 時間前** 〜 キックオフ | `saku_shiori_renofa_pre_match` |
| `inMatch` | キックオフ 〜 **+2 時間** | `saku_shiori_renofa_in_match` |
| `postMatch` | 試合終了の見込み 〜 **+2 時間** | `saku_shiori_renofa_post_match` |
| `finished` | それ以降（その日の残り） | `saku_shiori_renofa_post_match`（試合後のポーズのまま） |

- 置き場所は **`~/Library/Application Support/Enoki/Characters/saku_shiori_renofa_<phase>/`** です。
  クラブのエンブレムを含むので **リポジトリには入れません**（`saku_shiori_renofa` と同じ扱い）。
- **無ければ自動で落ちます**: 局面用のセットが見つからなければ `saku_shiori_renofa`、
  それも無ければベーススキン。ログ（category `appearance`）に info で出ます。
  4 セットを置かなくても、これまでどおり 1 セットだけで動きます。
- 生成（`--idle-image` で idle のコマを局面ポーズに差し替え、`--static-idle` でまばたきも同じ絵にする。
  手振り・居眠り・ドラッグなどのコマは従来のレノファ用シートから作ります）:

```bash
for ph in matchday pre_match in_match post_match; do
  python3 scripts/build_variant_atlas.py --src ~/Desktop/codex_pet_skin_sakushio --variant renofa \
    --idle-image ~/Desktop/codex_pet_skin_sakushio/renofa_$ph.png --static-idle \
    --out "$HOME/Library/Application Support/Enoki/Characters/saku_shiori_renofa_$ph" \
    --id sakushio_renofa_$ph --name "朔と栞（レノファ・$ph）"
done
```

#### ポーズ画像からスプライトセットを作る（`scripts/build_atlas.py`）

ポーズごとに 1 枚ずつある画像を、8 列 × 9 行のアトラスに詰める**汎用**スクリプトです。
全コマ共通の切り出し範囲・縮小率で、足元をセル下端 `--pad` px・水平中央に置くので、
コマ間で身長と立ち位置がずれません。`--map` でファイル名の対応表を差し替えられ、
足りないポーズは他の絵で埋めます。`--cell 320x208` のようにセルを広げると
`pet.json` に `mascot.grid` も書き出し、1 コマに複数人を入れたときは吹き出しの
`anchor` の目安も推定して表示します（docs/customization.md の 3 章・5 章）。
...
同梱のデモ（`Resources/DefaultSkin`）もこのスクリプトで作っています。
...
# 私服（ローカル専用。リポジトリには入れない）
python3 scripts/build_variant_atlas.py --src ~/Desktop/codex_pet_skin_sakushio --variant private \
    --out "$HOME/Library/Application Support/Enoki/Characters/saku_shiori_casual" --id sakushio_casual --name "朔と栞（私服）"
...
**このリポジトリには日程データを含めていません。** 開発者個人の環境に依存し、第三者のサイトから
写した内容を含むためです（`Sources/Enoki/Resources/Schedule/README.md`）。置き場所は次の 2 つで、
どちらにも無ければイベント日の判定は行われません（曜日と祝日だけで決まります）。

| 置き場 | 使いどころ |
|---|---|
| `~/Library/Application Support/Enoki/renofa-schedule.json` | **推奨・こちらが優先**。アプリを作り直さずに差し替えられる |
| `Sources/Enoki/Resources/Schedule/renofa-schedule.json` | 自分のビルドに同梱したい場合。`.gitignore` 済みでコミットされない |
...
```json
{
  ...
  "matches": [
    {"date": "2026-09-20", "kickoff": "13:00", "opponent": "対戦相手A", "home": false,
     "competition": "リーグ戦", "venue": "アウェイスタジアム"}
  ]
}
```
...
| `kickoff` | String? | `HH:mm`。**局面（`MatchPhase`）の基準**。台詞のカテゴリとスプライトセットがこの時刻で切り替わります。省略すると終日 `.matchDay` 扱い。書式が違えば時刻だけ捨てます。 |
| `opponent` | String | **必須**。対戦相手。 |
| `home` | Bool | ホームゲームなら `true`（メニューの `(H)` / `(A)`）。 |
| `competition` / `venue` / `note` | String? | 大会名・会場・メモ。表示用。 |

- 日程はシーズンが変わると古くなります。更新するときは**自分で確認できた日だけ**を書き、
  `source` に取得元（手で書いたなら `"manual"`）、`updated_at` に更新日を入れてください。
  推測した日程は入れない、が原則です（違う日に着替えてしまうため）。
- **アプリは日程を取りに行きません。** データは上の置き場に自分で置きます。
- 日程データを**リポジトリにコミットしないでください**。`.gitignore` で弾いていますが、
  第三者のサイト由来の内容を公開リポジトリに入れることになります。
...
- 試合日の**局面が変わったとき**（`AppearanceCoordinator` の局面タイマー。下の「キックオフ時刻への連動」）

の 4 か所で `reevaluateDay(reason:)` が走り、**手動選択の期限切れを処理してから**解決し直します。
...
- **キックオフ時刻への連動（実装済み）**: `RenofaMatch.phase(at:windows:calendar:)` が
  ...
  | 局面 | 窓 | 台詞カテゴリ |
  |---|---|---|
  | `.matchDay` | 00:00 〜 キックオフ 120 分前（キックオフ不明なら終日） | `renofa` |
  | `.preMatch` | キックオフ **120 分前**（`preMatchLead`）〜 キックオフ | `renofa_pre_match` |
...
- **見た目（スプライトセット）も局面に連動します（実装済み）**。処理経路は

  ```
    apply()
      └─ matchPhase(at: Date())            RenofaSchedule.phase(at:) → MatchPhase?（試合日でなければ nil）
      └─ profile.spriteSet(for: phase)     spriteSetsByPhase[rawValue] → 無ければ spriteSet
      └─ spriteSetSkin(for:phase:)         局面用セット → 通常のセット → ベーススキン の順にフォールバック
      └─ 解決先の URL が今と違えばフェード付きで差し替え（状態は .skinSwapped で継続）
  ```

  優先順位（手動選択 > その日の判定 > 仕事中モード連動 > default）は変わりません。
  局面で切り替わるのは**解決結果が `spriteSetsByPhase` を持つプロファイル（= `renofa`）のとき**だけなので、
  手動で Renofa を選んだ日でも、その日に試合があれば局面に合わせて着替えます。
- **境界の自動更新**: `RenofaMatch.phaseBoundaries(windows:calendar:)` が局面の切り替わり時刻
  （キックオフ −2 時間 / キックオフ / +2 時間 / +4 時間）を返し、
  `RenofaSchedule.nextPhaseBoundary(after:)` が「次の 1 つ」を返します。
  `AppearanceCoordinator` はその時刻（+1 秒）に**一度だけ**発火する `DispatchSourceTimer` を 1 本だけ持ち、
  発火したら `reevaluateDay(reason: "試合の局面が変わりました")` → `apply()` → 次の境界に張り直します
  （`start()` / `reevaluateDay` / `apply()` の最後で張り直し。**ポーリングはしません**）。
  試合日でない日・キックオフ不明・その日の境界を全部過ぎた後はタイマーを持ちません。
  スリープ中に境界を過ぎた場合は、復帰時の `reevaluateDay` で追いつきます。
  ログには「次の局面境界: 15:00」「試合の局面が変わりました → inMatch」と出ます。
